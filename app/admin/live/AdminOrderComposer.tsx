      ...
      {selectedProduct && (
        <div className="absolute inset-0 z-20 flex items-end justify-center bg-slate-950/55 p-0 sm:items-center sm:p-4">
          <div className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl sm:p-6">
            ...
            {selectedProduct.isCombo && selectedProduct.comboItemsConfig.map((comboItem) => comboItem.product.allowRemoveIngredients && comboItem.product.ingredients.length > 0 ? <OptionSection key={comboItem.id} title={`${comboItem.product.name} × ${(comboItem as any).pieces && (comboItem as any).pieces > 0 ? `${(comboItem as any).pieces} piezas` : comboItem.quantity}`}><div className="grid gap-2">{comboItem.product.ingredients.map((entry) => <CheckRow key={entry.ingredient.id} label={entry.ingredient.name} checked={!(comboRemoved[comboItem.id] ?? []).includes(entry.ingredient.id)} disabled={!entry.isRemovable} onChange={(checked) => setComboRemoved((current) => { const ids = current[comboItem.id] ?? []; return { ...current, [comboItem.id]: checked ? ids.filter((id) => id !== entry.ingredient.id) : [...ids, entry.ingredient.id] }; })} />)}</div></OptionSection> : null)}
