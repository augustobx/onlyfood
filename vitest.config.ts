...
export default defineConfig({
  test: {
    ...
    include: process.env.RUN_DB_TESTS === "true"
      ? ["tests/**/*.test.ts"]
      : ["tests/security-unit.test.ts", "tests/tenant-branding.test.ts", "tests/quantity-discounts.test.ts", "tests/whatsapp-notifications.test.ts", "tests/patty-count.test.ts", "tests/cashier-vip-store.test.ts", "tests/rewards-multi-unit-flow.test.ts", "tests/advance-order-scheduling.test.ts", "tests/sushi-theme.test.ts"],
    alias: {
